...
-- 9/10ths Lua script
-- By Garry Newman
-- Modified by Garry's Mod 9 Project
----------------------------------------------------------------------------------
-- Settings
...
--  Called right before the new map starts ------------------------------------
	...
	function gamerulesStartMap ()
		...
		fIntermissionEnd = 0
		RoundRestart();
		AddTimer( 3, 1, StorePropList, 0 ); -- Give the props time to settle before we capture their models, location and positions.
	end
	...
	function RespawnStoredProps()
		-- tprint( RespawnableProps ); -- Print a list of respawn props
		if ( table.getn(RespawnableProps) == 0 ) then return; end
		local CurrentEnts = _EntitiesFindByClass( "prop_physics" ); -- Remove all current props
		table.foreach( CurrentEnts, PropDelete );
		table.foreach( RespawnableProps, PropRespawn ); -- Spawn all the new props
	end
	...
	function RoundRestart( )
		...
		PlayerFreezeAll( false ); -- Unfreeze everyone
		PlayerSpawnAll( ); -- respawn all players
		...
		for i=1, _MaxPlayers() do
			_PlayerSetScore( i, 0 ) -- Reset indivdual scores to 0
		end
	end
	
	function doRoundTimer( ) -- called every second
...
-- Events --
	...
	function onEntityTouch( Activator, Caller, Team )
		...
		if ( IsPlayer(iPlayer) == false ) then
			return;
		end;
	...
	function ChooseTeam( playerid, num, seconds )
		...
		if (num == 2) then
			...
			return;
		end -- anything else is auto choose team
...
-- These are called by the players in game using the F1 - F4 keys
	...
	function onShowTeam ( userid )
		...
		_PlayerOption( userid, "ChooseTeam", 99999 ); -- I know this 99999 stuff sucks.. but so do I.
	end
